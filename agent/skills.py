...
# 弱知识点 → 所属科目（「记入学习记忆」写回时定位科目；只有能明确归属的才映射）
TOPIC_TO_SUBJECT = {
    "链表": "数据结构", "指针": "数据结构", "二叉树": "数据结构", "树": "数据结构", "图": "数据结构",
    "存储器": "计算机组成原理", "Cache": "计算机组成原理",
    "极限": "数学", "导数": "数学", "积分": "数学", "中值定理": "数学", "泰勒": "数学",
    "单词": "英语", "长难句": "英语",
}


def infer_subject_from_topics(topics):
    """从弱知识点反推科目；命中第一个有映射的 topic，否则返回空串。"""
    for t in topics or []:
        subj = TOPIC_TO_SUBJECT.get(t)
        if subj:
            return subj
    return ""
