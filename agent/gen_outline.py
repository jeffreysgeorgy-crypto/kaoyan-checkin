...
h(doc, "2.3.1 命题表面需求", 3)
p(doc, "【待补充：赛题原文逐条要求，将原文粘贴于此并对齐。】")
...
need(doc, "赛题原文完整要求（粘贴 + 逐条标注）", "行业权威数据来源（市场规模 / 增长率，标注出处）")
...
table(doc,
      ...
      [
          ["输入", "用户反思 / 碎碎念原文"],
          ["输出", "结构化信号：主因 / 次因 / 弱知识点 / 情绪 / 提炼结论（非原文复述）"],
          ["调用条件", "用户提交反思原文时（反思页「💬 分析反思」）"],
          ["关键逻辑", "优先 LLM 抽取（温度 0）；满足降级条件（未配 key / 超时 / 连续失败 3 次 / 解析失败）自动走关键词匹配"],
      ],
      widths=[3, 12])
...
p(doc, "后端 + 前端同源托管（Render 单域名），数据存 Supabase，本地开发经 cpolar 内网穿透调试；无额外硬件投入。")
