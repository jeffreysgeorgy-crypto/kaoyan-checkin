...
电脑跑前后端、手机通过 cpolar 隧道访问。cpolar 免费版地址**每次重启隧道都会变**，
所以前端「后端地址」做成了可配置（存在手机浏览器的 localStorage 里），换地址后改一次即可。

```text
1. 启动后端（绑定 0.0.0.0，供局域网/隧道访问）：
...
3. 启动前端静态服务（在 D:\XX 根目录，新开一个终端）：
   cd D:\XX && python -m http.server 8080

4. 给前端开隧道（再新开一个终端）：
   cpolar http 8080
   → 记下 cpolar 分配给前端的地址

5. 手机浏览器打开前端的 cpolar 地址，例如 https://yyyy.cpolar.cn/index.html

6. 进入「我的」页 → 「🔌 Agent 在线联动」卡片 → 「后端地址」输入框，
   把默认的 http://127.0.0.1:8000 改成第 2 步记下的 cpolar 地址（不要带末尾 /），点「保存」。

7. 点「📤 导出并发送 Agent」验证：状态栏变绿「已接收」即打通；
   若变红提示「后端未连接」，按提示检查 ①后端是否启动 ②地址是否填对 ③隧道是否还在运行。
```

> 注意：后端 CORS 已配置 `allow_origins=["*"]`，手机跨域访问无需额外改动；
> 前端所有请求都走 `BACKEND_URL` 变量（默认 `http://127.0.0.1:8000`，可在「我的」页改），不存在写死的 127.0.0.1。
